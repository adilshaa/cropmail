		...
		<div className="bg-white p-3 sm:p-4 md:p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow duration-200">
			...
			<div className="text-center">
				...
				<div className="text-xs text-gray-500">
					{icon}
					<span>
						{Math.round((limit - used).toLocaleString())} {unit} remaining
					</span>
				</div>
			</div>
		</div>
...
const Billing = () => {
	...
	return (
		<>
			<SEO
				title="Billing & Usage"
				description="Manage your Cropmail subscription, view usage statistics, and billing history."
				keywords={["billing", "usage stats", "email credits", "subscription management"]}
				canonical="https://cropmail.onrender.com/home/billing"
			/>
			<div className="w-full p-2 sm:p-4 md:p-6 space-y-4 md:space-y-6">
								...
								<table className="min-w-full divide-y divide-gray-200">
									<thead>
										<tr className="bg-gray-50">
											<th className="px-3 sm:px-4 md:px-6 py-3 md:py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
												Date
											</th>
											<th className="px-3 sm:px-4 md:px-6 py-3 md:py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
												Invoice
											...
											<th className="px-3 sm:px-4 md:px-6 py-3 md:py-4 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">
												Actions
											</th>
										</tr>
									</thead>
									<tbody className="bg-white divide-y divide-gray-200">
										{billingHistory.map((bill) => (
											<tr key={bill.id} className="hover:bg-gray-50">
												<td className="px-3 sm:px-4 md:px-6 py-3 md:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500">
													{bill.date}
												</td>
												<td className="px-3 sm:px-4 md:px-6 py-3 md:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500">
													{bill.invoice}
				...
				<div className="bg-white rounded-lg shadow-md">
					...
					>
						...
						<svg
							className={`w-6 h-6 transform ${
								isPreviousPlansOpen ? "rotate-180" : ""
							} transition-transform duration-200`}
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
